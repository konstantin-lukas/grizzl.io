...
class Container {
    ...
    resolve<T>(injectable: InjectableClass<T>, resolvingStack = new Set<Constructor<unknown>>()): T {
        ...
        const depsInstances = (injectable.deps || []).map(dep => this.resolve(dep, resolvingStack));
        const instance =
