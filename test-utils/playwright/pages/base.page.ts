...
export default abstract class BasePage<T extends Record<string, string>> {
    ...
    private async toBeValid(
        options:
            | { name: string; skipThemeToggle?: boolean; blur?: boolean }
            | { screenshotName: string; ariaName: string; skipThemeToggle?: boolean; blur?: boolean },
        target?: Locator | Page,
    ) {
        ...
        await this.toHaveScreenshot(target ?? this.page, { name: screenshotName!, blur: options.blur });
        this.toHydrate();
        this.toHaveNoErrors();
        if (skipThemeToggle) return;
        await this.toggleTheme();
        await this.toHaveScreenshot(target ?? this.page, { name: `${screenshotName}-darkmode`, blur: options.blur });
        await this.toggleTheme();
    }
