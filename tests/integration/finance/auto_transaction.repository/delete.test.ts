...
test("deletes only resources owned by the user", async ({ db, user }) => {
    ...
    const [autoTransaction1] = await db.financeAutoTransaction.insert(1, { accountId: account1.id });
    const [autoTransaction2] = await db.financeAutoTransaction.insert(1, { accountId: account2.id });
