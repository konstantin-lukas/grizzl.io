...
test("only allows a user to edit their own resources", async ({ request, db }) => {
    ...
    const [data] = await db.timer.insert(1, { userId: otherUser!.id });
    expect(data.deletedAt).toBeNull();
    const response = await request.patch(`${route}/${data.id}`, { data: { deleted: true } });
    expect(response.status()).toBe(404);
    const [patchedTimer] = await db.timer.select(data.id);
    expect(patchedTimer!.id).toBe(data.id);
});

test("only allows patching the deleted property", async ({ request, db }) => {
    const [data] = await db.timer.insert(1);
    const response = await request.patch(`${route}/${data.id}`, { data: { ...FULL_TIMER, deleted: true } });
    expect(response.status()).toBe(204);
    const [patchedData] = await db.timer.select(data.id);
...
test("only modifies the requested resource", async ({ request, db }) => {
    const otherUser = await db.user.selectByEmail("cmontgomeryburns@springfieldnuclear.com");
    const [otherData] = await db.timer.insert(1, { userId: otherUser!.id });
    const [data] = await db.timer.insert(1);
    expect(data.deletedAt).toBeNull();
    expect(otherData.deletedAt).toBeNull();
    await request.patch(`${route}/${data.id}`, { data: { deleted: true } });
    const [dataAfterPatch] = await db.timer.select(data.id);
    const [otherDataAfterPatch] = await db.timer.select(otherData.id);
...
test("allows undoing a delete", async ({ request, db }) => {
    const [data] = await db.timer.insert(1, { deletedAt: new Date() });
    expect(data.deletedAt).not.toBe(null);
    await request.patch(`${route}/${data.id}`, { data: { ...BASE_TIMER, deleted: false } });
    const [patchedData] = await db.timer.select(data.id);
