...
test.each([
    ...
    { name: "should return the string when it the same length as max", text: "short", max: 5, expected: "short" },
    { name: "should return the string when it is shorter than max", text: "short", max: 10, expected: "short" },
    {
        name: "should truncate the string and add an ellipsis when it is on charater longer than max",
        text: "short",
        max: 4,
        expected: "shor…",
    },
