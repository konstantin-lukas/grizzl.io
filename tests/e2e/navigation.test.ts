...
withoutAuth(() => {
    test(`redirects the user to /timer after signing in through a client-side redirect`, async ({ homePage: page }) => {
        await page.goto();
    ...
    const protectedPaths = ["/timer", "/finance", "/todo", "/todo/calendar", "/poll"];
